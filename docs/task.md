...
### 6.0.1 关键产品决策(2026-07 与运营者确认,后续实现以此为准)

| # | 决策 | 结论 | 理由 |
|---|------|------|------|
| D1 | 采集频率粒度 | **全局统一**,不做"每个博主/关键词单独设频率" | 运营侧不需要差异化;一个全局 cron 表达式即可 |
| D2 | 博主首次采集深度 | **首次只采最近 N 条**(`CREATOR_FIRST_FETCH_LIMIT`,默认 50),不拉全量历史 | 大博主上千条,Apify 按量计费 + worker 解析成本会爆炸;最近 N 条足够建基线 |
| D3 | 采集后解析策略 | **采回来的视频全部自动解析**(下载 MP4 + Gemini) | 博主/关键词都是主动选的高信任源;总量已由 D2 + 筛选条件控住 |
| D4 | 老视频数据刷新范围 | **只刷新"本轮 Apify 返回的、库里已存在的"那批**,不做全库回刷 | Apify `profiles` 每次只返回最新 N 条,天然限定范围;全库回刷量费不可控 |
| D5 | 博主视频列表取数键 | **按 `author_id` 查**,不按 `source_type` | 同一条视频可能被博主监控 + 关键词都采到,`source_type` 只记先到的那个;按作者查才不漏 |
| D6 | 单条 URL 重复解析 | **命中已存在 `tiktok_video_id` → 直接跳到已有详情**,不重复下载解析 | 复用结果,省成本;解析结果与来源无关 |
| D7 | "新采集"标记语义 | **按入库时间近似**(如 24h 内),不做"已读/未读" | 无用户系统,做不了 per-user 已读状态 |
...
#### 数据模型变更
- [x] `creators` 表:加 `video_count`(该博主已采集视频数)、`last_fetch_video_count`(上次采集新增数),方便卡片展示统计
- [x] `creators` 表:`monitor_frequency` 改为**全局统一设置**(不存在每个博主不同频率),存到 settings 或环境变量 `CREATOR_FETCH_CRON`(如 `0 */1 * * *` 每小时)
- [x] `creators` 表:`status` 支持 `active` / `paused`(暂停/启用,已有字段,差 API)

#### API
- [x] `POST /api/creators` 放宽输入:接受 `@username` / `username` / 完整 URL,后端统一补全成 `https://www.tiktok.com/@username`
- [x] `PATCH /api/creators/[id]` 新增:切换 `status`(active ↔ paused)
- [x] `GET /api/creators/[id]/videos` 新增:该博主名下所有视频(带解析状态 + 指标)。**按 `videos.author_id = creators.creator_id` 查(D5)**,不按 source_type —— 保证被关键词入口采到的同作者视频也能显示

#### 采集逻辑(`monitor-creators` cron 改造)
- [x] **首次订阅 = 采最近 N 条**(D2):Apify `profiles` 抓该博主最近 `CREATOR_FIRST_FETCH_LIMIT`(默认 50)条,全部入库(source_type='creator_monitor'),全部进入解析 pipeline(D3)
- [x] **后续周期 = 增量 + 数据刷新**:
  - [x] Apify `profiles` 每次返回最新一批(resultsPerPage 条)
  - [x] 其中**新视频**(库里没有的 tiktok_video_id)→ 入库 + 进解析 pipeline
  - [x] 其中**老视频**(库里已存在的)→ 只刷新互动数据(play/like/comment/share/collect),不重新解析
  - ⚠️ 刷新范围严格限定"本轮 Apify 返回的这批"(D4),不做全库回刷
- [x] 采集时跳过 `status='paused'` 的博主
- [x] 采集完更新 `last_fetch_time` + `last_fetch_video_count`(本轮新增数)
...
- [x] 每个博主 = 一个**订阅卡片**,展示:
  ...
  - 操作:暂停/启用、删除
- [x] 添加博主对话框:输入 `@username` 或 URL,可选填分类
- [x] 卡片可展开/点击 → 进入**该博主的视频列表页**

#### 前端:博主详情页(`/creators/[id]`)— 新页面
- [x] 顶部:博主信息(头像/名称/统计/状态)
- [x] 主体:该博主采集到的所有视频列表(表格或卡片网格)
  - 每条:封面 + 标题 + 解析状态 badge + 播放/点赞 + 采集时间
  - 解析中/异常的视频灰显,解析完成的可点击进详情
- [x] 新采集的视频高亮标记(如"NEW" badge 或排序置顶)
...
#### 数据模型变更
- [x] `keywords` 表加采集筛选条件字段(订阅时设置,采集时应用):
  - `min_play_count` INTEGER — 最低播放量门槛(如 ≥10000)
  - `min_like_count` INTEGER — 最低点赞门槛
  - `min_engagement_rate` NUMERIC — 最低互动率门槛(like_count/play_count,如 0.05=5%)。比绝对数更能筛黑马,不偏大号
  ...
  - `exclude_slideshow` BOOLEAN — 只要视频、排除图文(slideshow 无 mp4,解析会降级),默认 true
- [x] `keywords.monitor_frequency` 同博主,改为全局统一设置 `KEYWORD_FETCH_CRON`

#### 采集逻辑(`search-keywords` cron 改造)
- [x] Apify `searchQueries` 抓搜索结果
- [x] **按筛选条件过滤**(入库前):
  - play_count ≥ min_play_count?
  - like_count ≥ min_like_count?
  - like_count/play_count ≥ min_engagement_rate?(互动率门槛)
  - publish_time ≥ published_after?
  - duration 在范围内?
  - 不含 unwanted_hashtags?
  - exclude_slideshow=true 时,isSlideshow 视频跳过?
  - 不满足任一条件 → 跳过不入库(省解析成本)
- [x] 通过筛选的 → 去重(tiktok_video_id)→ 入库(source_type='keyword_search')→ 解析 pipeline
- [x] 采集完更新 `last_fetch_time`

#### 前端:关键词列表页(`/keywords`)
- [x] 每个关键词 = 一个**订阅卡片**,展示:
  - 关键词文本 + 地区 + 语言
  - 筛选条件摘要(如"播放≥1万 · 互动率≥5% · 近7天 · 15-60秒 · 仅视频")
  - 统计:已采集 N 条 / 已解析 M 条 / 上次采集时间
  - 状态开关 + 删除
- [x] 添加关键词对话框:输入关键词 + 设置筛选条件(播放量/时间/时长/排除标签)
- [x] 卡片点击 → 该关键词的视频列表页

#### 前端:关键词详情页(`/keywords/[id]`)— 新页面
- [x] 同博主详情页结构:关键词信息 + 该关键词采集到的视频列表

---

### 6.3 单条视频解析(保留)🟡

- [x] 首页或顶部保留一个**单条解析入口**:粘贴 TikTok 视频 URL → 一次性解析
- [x] 单条解析不走订阅/采集,直接 `POST /api/tasks` { task_type: 'analyze_video' } → 解析 pipeline → 完成后在视频库可见
- [x] **重复解析处理(D6)**:提交前按 `tiktok_video_id` 查库,已存在则直接跳到已有详情页,不重复下载解析(复用结果)
- [x] 单条解析的视频 `source_type='manual_video'`,在视频库可按来源筛选区分
...
- [x] 素材区下方才是 8 区块分析结果(现有 analysis-view)
- [x] 解析失败时:素材区显示错误原因(error_message)+ 重试按钮
- [x] 解析中时:素材区显示进度文案 + 自动轮询(已有 pending-analysis-panel 逻辑)
...
- [x] **筛选**:按来源(source_type: creator_monitor / keyword_search / manual_video)、解析状态、博主、关键词
- [x] **搜索**:按标题/作者搜索
- [x] **"新采集"标记**:最近 24 小时内入库的视频高亮(NEW badge 或排序置顶 + 视觉区分)
- [x] **排序**:按采集时间 / 播放量 / 点赞数
- [x] 表格列优化:封面 + 标题 + 来源(博主/关键词图标)+ 播放/点赞 + 解析状态 + 采集时间
...
首页从"提交入口"升级为"工作台汇总":
- [x] **汇总数字**:已订阅博主 N 个 / 已订阅关键词 M 个 / 视频库共 X 条 / 今日新增 Y 条 / 待解析 Z 条
- [x] **最近活动**:最近采集的视频(缩略图列表,5-10 条)
- [x] **解析状态**:当前正在解析的视频进度
- [x] **快速解析入口**:单条 URL 解析(保留,但降级为次要位置)

---

### 6.7 清理 Vercel 时代遗留 🟡

- [x] 删除 `creator-card.tsx` / `keyword-card.tsx` 的"立即抓取/搜索"按钮(改为自动定时,不需要手动触发)
- [x] 删除 `pending-analysis-panel.tsx` 的 60s "踹一脚" 兜底(Railway 常驻进程,pipeline 接力更可靠)
- [x] 设置页"手动触发"区块改为"调度状态"展示(上次运行时间 / 下次预计 / 各任务状态)
- [x] 删除前端所有 `fetch('/api/cron/...')` 的直接调用(cron 由 Railway 定时触发,不再前端调)
...
Railway Cron Trigger(UTC)
  ...
  │    遍历 active 博主 → Apify profiles 抓取(首次最近 N 条 / 后续最新一批)
  │    → 新视频入库+解析 / 老视频仅刷新本轮返回的那批数据
  │
